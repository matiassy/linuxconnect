...
{
  global $dominio;
  global $config, $defaultLowUser, $defaultHighUser;
...
class host
{
  ...
  var $lowUser = 'lunix';
  var $highUser = 'root';
  function __construct($ip, $name, $port = 22, $id, $hop = NULL, $lowUser = 'lunix', $highUser = 'root')
  {
    $this->ip = $ip;
    $this->name = $name;
    $this->port = $port;
  ...
  function _loginLowUser($asHighUser, $destino)
  {
    ...
    if (!isset($this->account[$this->lowUser]) || $this->account[$this->lowUser] === '') {
      debug("ERROR: No hay password para el usuario '{$this->lowUser}' en '{$this->id}'\n");
      return 0;
    }
